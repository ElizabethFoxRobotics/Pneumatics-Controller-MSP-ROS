...
void sensorUpdate(void) {
    ...
    //!!! something wrong here pressure always ~60
    ...
    for (int i = 0; i < NUM_PRES_SENSOR; i++) { // loop over pressure sensors
        ...
        if(i==0){
            pres[i][0] = convert.intToFloat(adc_conv[idx_adc], PRES_SENSE_TANK)*6.89475729; // convert and update pressure, psi>kpa
        }
        else{
            pres[i][0] = convert.intToFloat(adc_conv[idx_adc], PRES_SENSE_INT)*6.89475729; // convert and update pressure, psi>kpa
        }
        // calculate filtered pressures
        presFilt[i][1] = presFilt[i][0]; // shift filtered pressure histories
        presFilt[i][0] = pres[i][0];//0; // reset current value to zero
...
//    }
    ...
    uart_tx[1] = pres_des[1];
    uart_tx[2] = presFilt[1][0];

        /*
         for (int i=1; i< NUM_PRES_SENSOR; i++){
        uart_tx[3*i-2] = pres_des[i];
        uart_tx[3*i-1] = presFilt[i][0];
    }*/
