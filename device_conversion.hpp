...
#define PRES_SENSE_TANK 5
#define PRES_SENSE_INT 6
...
Converter::Converter(float adc_resolution, float volt_supply, float volt_adc) {
  ...
  sensor_lib[PRES_SENSE_TANK].slope = (v_adc/adc_res)*((100)/(0.8*v_supply));
  sensor_lib[PRES_SENSE_TANK].offset = -(0.34*(100))/(0.8*v_supply);
  sensor_lib[PRES_SENSE_INT].slope = (v_adc/adc_res)*((100)/(0.8*v_supply));
  sensor_lib[PRES_SENSE_INT].offset = -(.1535*(100))/(0.8*v_supply);
